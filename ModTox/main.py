...
def parse_args():
    ...
    return args.traj, args.resname, args.active, args.inactive, args.top, args.glide_files, args.model, args.best, args.csv, args.RMSD, args.cluster, args.last, args.clust_type, args.rmsd_type, args.receptor, args.ligands_to_dock, args.grid, args.precision, args.maxkeep, args.maxref, args.dock, args.analysis, args.test, args.save, args.load, args.external_data

def main(traj, resname, active, inactive, top=None, glide_files="*dock*.mae", model=False, best=True, csv=False, RMSD=True, cluster=True, last=True, clust_type="BS", rmsd_type="BS", receptor="*pv*.maegz", grid=None, precision="SP", maxkeep=500, maxref=400, dock=False, analysis=True, test=None, save=None, load=None, external_data=None):
    if dock:
        ...
        print("Docking in process... Run the same command with --analysis flag to build model")
    elif analysis:
        # Analyze dockig files and build model features
        gl.analyze(glide_files, model=model, best=best, csv=csv, active=active, inactive=inactive)
        # Build Model
        clf = svm.SVC(C=1, gamma=1, kernel="linear")
        model = md.GenericModel(active, inactive, clf, csv=external_data, test=test)
        model.fit_transform(cv=2)
        print("Model sucesfully build. Confusion_matrix.png outputted")



if __name__ == "__main__":
    trajs, resname, active, inactive, top, glide_files, model, best, csv, RMSD, cluster, last, clust_type, rmsd_type, \
       receptor, ligands_to_dock, grid, precision, maxkeep, maxref, dock, analysis, test, save, load, external_data = parse_args()
    main(trajs, resname, active, inactive, top, glide_files, model, best, csv, RMSD, cluster, last, clust_type, rmsd_type, 
        receptor, grid, precision, maxkeep, maxref, dock, analysis, test, save, load, external_data)
