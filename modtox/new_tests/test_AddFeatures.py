...
from modtox.modtox.new_models_classes.Features.add_feat import AddGlide, AddTopologicalFingerprints, AddMordred
...
def test_format_glide():
    g = AddGlide("a", "b", "c", glide_csv=GLIDE_CSV)  # Create object to access staticmethod
...
def test_add_topo():
    ...
    assert all(isinstance(mol.topo, rdkit.DataStructs.cDataStructs.ExplicitBitVect) for mol in [mol1, mol2])
