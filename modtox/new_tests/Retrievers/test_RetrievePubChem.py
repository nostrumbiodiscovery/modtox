...
def test_download_csv_successful():
    ...
    assert u.get_latest_file("*") == os.path.join(
        os.getcwd(), "PROTACXN_P23316_bioactivity_protein.json"
    )


@pytest.mark.parametrize(
    "input,expected", [(19, "InChI=1S/C7H6O4/c8-5-3-1-2-4(6(5)9)7(10)11/h1-3,8-9H,(H,10,11)"), (33, "InChI=1S/C2H3ClO/c3-1-2-4/h2H,1H2"),]
)
def test_cid2inchi(input, expected):
    r = RetrievePubChem()
    assert r._cid2inchi(input) == expected


def test_cid2_inchi_unsuccessful():
    r = RetrievePubChem()
    with pytest.raises(pubchempy.BadRequestError):
        inchi = r._cid2inchi(891744216472167260)
...
def test_parse_activity(mock):
    ...
    assert r.ids[activity.inchi] == "56661147"
