...
class ProcessorSDF:
    ...
    def _load_train_set(self, sdf_active, sdf_inactive, sdfs_to_compare):
        ...
        self.mol_names = []
        actives_non_repeated = []
        inactives_non_repeated = []

        for mol in actives:
            mol_name = mol.GetProp("_Name")
            if mol_name not in self.mol_names:
                self.mol_names.append(mol_name)
                actives_non_repeated.append(mol)

        for mol in inactives:
            mol_name = mol.GetProp("_Name")
            if mol_name not in self.mol_names:
                self.mol_names.append(mol_name)
                inactives_non_repeated.append(mol)

        # keep balance sets
        balance = False
        if balance:
            # random balance
            randoms = np.random.choice(range(max(len(inactives_non_repeated), len(actives_non_repeated))),
                                       min(len(inactives_non_repeated), len(actives_non_repeated)), replace=False)

            if len(inactives_non_repeated) > len(actives_non_repeated):
                inactives_non_repeated = np.array(inactives_non_repeated)[randoms]
                randoms = np.add(randoms, len(actives_non_repeated))
                idxs = np.concatenate((list(range(len(actives_non_repeated))), randoms))
            else:
                idxs = np.concatenate((randoms, list(range(len(actives_non_repeated), len(self.mol_names)))))
                actives_non_repeated = actives_non_repeated[randoms]
            self.mol_names = [mol for i, mol in enumerate(self.mol_names) if i in idxs]

        # Main Dataframe
        actives_df = pd.DataFrame({TITLE_MOL: actives_non_repeated})
        inactives_df = pd.DataFrame({TITLE_MOL: inactives_non_repeated})
    ...
    def transform(self, folder, exclude=COLUMNS_TO_EXCLUDE):
        ...
        if self.external_data:
            ...
            features.extend([('external_descriptors',
                              ExternalData(self.external_data, self.mol_names, exclude=exclude, folder=folder))])
    ...
    def transform_mol(self, folder, exclude=COLUMNS_TO_EXCLUDE):
        assert self.fitted, "Please fit the processor first."
    ...
    def sanitize(self, X, y, cv, feature_to_check='external_descriptors', folder='.'):
        ...
        if feature_to_check == 'external_descriptors':
            ...
            molecules_to_remove = [mol for i, mol in enumerate(self.mol_names) if
                                   np.isnan(X[i, indices_to_check]).all()]
