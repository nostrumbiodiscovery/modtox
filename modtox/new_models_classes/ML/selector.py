...
class ChiPercentileSelector(FeaturesSelector):
    ...
    def select(self):
        ...
        X_new = selector.fit_transform(X_abs)
        
        support = selector.get_support()  # T/F array -> T: feature kept, F: feature dropped
        columns_kept = [col for col, tf in zip(X_abs.columns, support) if tf == True]  # Columns 
        new_df = pd.DataFrame(X_new, columns=columns_kept)
        # Append two columns to maintain same structure as original dataframe
        new_df["Activity"] = self.dataset.activity
        new_df["is_external"] = self.dataset.is_external
        self.dataset.df = new_df
        return self.dataset.df.shape[1]
